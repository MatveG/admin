...
export default {
  name: 'ClientForm',
  components: { UserAvatar, FilePicker, CardComponent, Tiles, Notification },
  props: {
    id: {
      default: null
    }
    ...
    getData () {
      if (this.id) {
        axios
          ...
          .then((r) => {
            const item = r.data.data.find((item) => item.id === parseInt(this.id));

            if (item) {
              this.isProfileExists = true
              this.form = item
              this.form.created_date = new Date(item.created_mm_dd_yyyy)
              this.createdReadable = '2000-01-01'
            } else {
